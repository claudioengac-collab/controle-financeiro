...
async function buildWeb(domain) {
  console.log("Building web version (expo export --platform web)...");
  return new Promise((resolve, reject) => {
    ...
    if (webProc.stdout) webProc.stdout.on("data", (d) => process.stdout.write(d));
    if (webProc.stderr) webProc.stderr.on("data", (d) => process.stderr.write(d));
...
async function main() {
  ...
  try {
    ...
    console.warn("Native bundle step skipped:", nativeError.message);
    console.log("Web build is available. Continuing with web-only deployment.");
  }
...
main().catch((error) => {
  ...
  if (webBuildExists()) {
    console.warn("Build error (non-fatal, web build exists):", error.message);
    process.exit(0);
  }
